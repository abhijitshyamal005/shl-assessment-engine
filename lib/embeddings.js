...
// Lazy load transformer pipeline only when needed to avoid bundling issues
// @xenova/transformers has native dependencies (sharp, etc) that conflict with Next.js builds
let pipeline = null;

async function loadPipeline() {
  if (pipeline !== null) return pipeline; // already loaded or tried
  try {
    const transformers = await import('@xenova/transformers');
    pipeline = transformers.pipeline;
    return pipeline;
  } catch (err) {
    console.warn('Transformers not available, using software embeddings:', err?.message || err);
    pipeline = false; // mark as tried but failed
    return null;
  }
}
